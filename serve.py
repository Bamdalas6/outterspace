...
variants_map = {}
for idx, p in enumerate(products_list):
    base_id = 47945000000000 + (idx * 100)
    for s_idx, size_name in enumerate(['1', '2', '3']):
        var_id = base_id + (s_idx + 1)
        variants_map[str(var_id)] = {
            'product': p,
            'size': size_name,
            'variant_id': var_id,
        ...
        if path in ['/cart/add.js', '/cart/add']:
            ...
            if not var_info and len(products_list) > 0:
                ...
                var_info = {
                    'product': p0,
                    'size': '1',
                    'variant_id': 47945000000001,
                    'price': int(float(p0['price']) * 100)
                }

            if var_info:
                prod = var_info['product']
                img_src = f"/{prod['images'][0]}" if prod.get('images') else ""
                
                found = False
                for it in active_cart['items']:
                    if it['id'] == var_info['variant_id']:
                        it['quantity'] += qty
                ...
                if not found:
                    new_item = {
                        ...
                        "key": f"{var_info['variant_id']}:12345678",
                        "title": prod['title'],
                        ...
                        "image": img_src,
                        "handle": prod['handle'],
                        "product_type": prod.get('category', ''),
